...
public class forEachClassActivity {
	...
	public static void main(String[] args) {
		...
		for(int x = 0; x < age.length; x ++){
			System.out.println("What is the age of your student?");
			age[x] = input.nextInt();
		}
		for (int num : age){
			sum = sum+num;	
		...
		*/
		...
		for(int x = 0; x<workers; x ++){
			System.out.println("How much is your worker paid?");
			wages.add(input.nextDouble());
		}
		for(double num : wages){
			if(num>max){
				max = num;
			}
		}
		...
		*/
		...
		for(int x = 0; x < 10; x ++){
			System.out.println("What is the radii");
